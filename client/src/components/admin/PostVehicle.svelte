...
<form>
  ...
  <div>
    ...
    <input id="file" type="file" placeholder="Select Files" class="input input-bordered input-primary w-full max-w-xs" bind:files/>
  </div>
  <div>
    <button type="submit" on:click={doPost} class="bg-accent px-5 py-2.5 text-center">Submit</button>
  </div>
  </div>
</form>